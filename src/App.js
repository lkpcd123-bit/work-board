            ...
            <div className="modal-body" style={{maxHeight:"70vh",overflowY:"auto",display:"flex",flexDirection:"column",gap:10,padding:"0 24px 24px"}}>
              ...
              {tabFolders.map((folder)=>(
                <div key={folder.id} style={{border:"1.5px solid var(--line)",borderRadius:12,padding:"16px 18px",background:"var(--bg)"}}>
                  ...
                  {!(folder.tabs||[]).length&&<div style={{fontSize:12,color:"var(--ink3)"}}>비어있음 — 표시 중 탭에서 📁 폴더로 버튼으로 추가하세요</div>}
                </div>
              ))}
                  ...
                  <div style={{padding:"16px 20px"}}>
                    {hist.length>1&&(
                      <div style={{marginBottom:16}}>
                        <div style={{fontSize:12,fontWeight:700,color:"var(--ink3)",marginBottom:8}}>재고 추이</div>
                      ...
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                        <thead>
                          <tr style={{background:"#F4F5F7"}}>
                            <th style={{padding:"7px 12px",textAlign:"left",fontWeight:700}}>날짜</th>
                            <th style={{padding:"7px 12px",textAlign:"right",fontWeight:700}}>재고</th>
                            <th style={{padding:"7px 12px",textAlign:"right",fontWeight:700}}>전일대비</th>
                          </tr>
                        </thead>
                        <tbody>
            ...
            {[{id:"schedule",label:"📅 스케줄"},{id:"editor",label:"✏️ 상품 편집기"},{id:"ytlink",label:"🎬 유튜브링크"}].map((t)=>(
              <button key={t.id} onClick={()=>setC24SubTab(t.id)}
                style={{padding:"10px 18px",border:"none",cursor:"pointer",fontSize:13,fontWeight:c24SubTab===t.id?800:500,
                  ...
                  borderBottom:c24SubTab===t.id?"3px solid #0C66E4":"3px solid transparent",
...
function YtLinkPanel({c24Api, c24TokenValid, c24GetProduct}) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // 3) 이미지 없이 상품 먼저 생성 (이미지는 PUT으로 별도 처리)
      setMsg("새 상품 생성 중...");
      const createPayload = {
      ...
      // 4) 카테고리 설정 (PUT)
      setMsg("카테고리 설정 중...");
      const catRes = await c24Api({action:"update", productNo:newNo, payload:{
        category:[{category_no:247,recommend:"F",new:"F"}]
      }});
      if(!catRes.product) setMsg("⚠ 카테고리 설정 실패: "+JSON.stringify(catRes));
      ...
      const uploadImg = async (url) => {
        ...
        if(!path) setMsg("⚠ 이미지 업로드 응답: "+JSON.stringify(r).slice(0,100));
        return path;
      };
      ...
      const imgCount = [mainImage,listImage,tinyImage,smallImage].filter(Boolean).length;
